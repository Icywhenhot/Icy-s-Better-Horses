...
On Fabric everything built in lives under one namespace. Prefer the supplied constants and registry objects instead of constructing built-in IDs yourself.

| Purpose | Value |
| --- | --- |
| Mod ID, registries, built-in entries, assets and equipment tags | `icys-better-horses` |
| Your addon entries and assets | Your own mod ID, such as `trailaddon` |

`BhRegistries` exposes Fabric registries for breeds, archetypes, abilities, commands, genders, and species. Register into them from your mod initializer with `Registry.registerForHolder` and keep the returned `Holder.Reference`. Do not replace built-in registry entries or register under another mod's namespace, obviously. Add `icys-better-horses` to the `depends` block of your `fabric.mod.json`.
...
public final class TrailAddon implements ModInitializer {
    public static final String ID = "trailaddon";

    public static Holder.Reference<AbilityType> RECOVER;
    public static Holder.Reference<CommandType> HALT;

    @Override
    public void onInitialize() {
        RECOVER = Registry.registerForHolder(BhRegistries.abilityTypeRegistry(),
                Identifier.fromNamespaceAndPath(ID, "recover"), new AbilityType(Recover::new, true));
        HALT = Registry.registerForHolder(BhRegistries.commandTypeRegistry(),
                Identifier.fromNamespaceAndPath(ID, "halt"), new CommandType(
                        (horse, player) -> IHorseData.of(horse).bh_getBond() >= 20,
                        (horse, player) -> {
                            horse.getNavigation().stop();
                            IHorseData.of(horse).bh_setCommand(BhContent.COMMAND_STAY.key());
                        }));
        BhContent.SHIRE.value().addAbility(RECOVER.key());
    }
}
```
...
Call `BreedType.addAbility(key)` from your mod initializer, which runs on both client and server. It appends without removing built-in abilities and ignores duplicate additions. `abilities()` returns an immutable snapshot.
...
For example, put this in `data/icys-better-horses/tags/item/gear/hooves.json`:
...
Register a listener on `HorseFeaturesEvent.EVENT`. It is posted once per managed horse's feature-list initialization on either logical side. Add a new instance of your `HorseFeature` for that horse:

```java
icy.betterhorses.net.api.HorseFeaturesEvent.EVENT.register(event -> {
    event.add((horse, data) -> {
        if (horse.level().isClientSide() || horse.tickCount % 100 != 0) return;
        if (horse.isInWater()) horse.heal(0.5F);
    });
});
```

Register the listener once from your mod initializer. `HorseFeature` also has `onLoad`, `onRemoved`, and `onInventoryChanged` hooks. Features append after the built-in feature list. Do not share a mutable feature instance across horses.

Feature initialization can happen while the horse's inventory or saved data is being restored. Do not assume all breed or equipment fields are final inside the attachment event; evaluate changing eligibility inside your feature methods. Features can run on both sides, so guard server-only gameplay work. For feature persistence use your own Fabric data attachment; automatic per-instance save/load in this API belongs to `BreedAbility`.

## Register a new breed

Register into `BhRegistries.breedTypeRegistry()`, and register an entity type for a subclass of `BhBreedHorse`. Implement `bhFixedBreed()` to return your breed's key. `BhBreedHorse` supplies coat storage and inheritance; `bhAttributes(archetype)` supplies the matching starting attribute builder.

A breed registration has this shape, where `TRAIL_HORSE_KEY` is your entity type's `ResourceKey` and `RECOVER` is your registered ability:

```java
Registry.registerForHolder(BhRegistries.breedTypeRegistry(),
        Identifier.fromNamespaceAndPath("trailaddon", "trail_horse"),
        BreedType.builder(BhContent.DRAFT.key())
                .coats("trailaddon", "trail_horse", java.util.List.of("bay", "black"), true)
                .entityType(TRAIL_HORSE_KEY)
                .chestRows(4)
                .bondedChestRows(4)
                .ability(RECOVER.key())
                .build());
...
Register the entity's attributes with `FabricDefaultAttributeRegistry`, and register its renderer and model layers on the physical client. Review `PercheronHorse` and `PercheronHorseRenderer` for the existing entity and tack-rendering pattern. Breed registration does not generate models, entity registration, renderers, spawn rules, sounds, or spawn eggs for you.
