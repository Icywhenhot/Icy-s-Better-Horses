...
public final class HorseStabilizerLogic {
    ...
    // Note: verticalSpeed is retained in the signature for backwards compatibility but is no longer consulted. On a player-ridden horse the server never updates the horse's own deltaMovement.y (the rider's client drives positional movement and the horse is dragged along), so verticalSpeed stays at 0.0 the entire fall — which previously caused the deploy gate to short-circuit to CLOSED and the wings to never open. fallDistance is updated correctly via positional deltas and is the reliable signal.
