...
public class IcysBetterHorsesClient {
    ...
    public IcysBetterHorsesClient(IEventBus modEventBus, ModContainer modContainer) {
        ...
        modEventBus.addListener(this::registerClientHandlers);
    ...
    private void registerClientHandlers(RegisterClientPayloadHandlersEvent event) {
        event.register(HorseRosterSyncPayload.TYPE, (payload, context) ->
                ClientHorseRoster.setEntries(payload.entries()));

        event.register(HorseManageResultPayload.TYPE, (payload, context) ->
        ...
        event.register(TrustSyncPayload.TYPE, (payload, context) ->
                ClientTrustCache.set(payload.trustingOwners()));

        event.register(HorseChargeShakePayload.TYPE, (payload, context) ->
                ChargeShakeController.trigger());

        event.register(ConfigSyncPayload.TYPE, (payload, context) -> {
        ...
        event.register(BreedDataPayload.TYPE, (payload, context) -> {
            if (Minecraft.getInstance().hasSingleplayerServer()) return;
    ...
    private void onClientTick(ClientTickEvent.Post event) {
        ...
        if (callKeyDown && !callKeyWasDown) {
            if (bh_anyHorseRoused(client)) {
                ClientPacketDistributor.sendToServer(new HorseRecallPayload());
    ...
    private static void bh_tryRear(Minecraft client) {
        ...
        ClientPacketDistributor.sendToServer(new BhRearPayload(horse.getId()));
    }
    ...
    private static void bh_trySwapCartSize(Minecraft client) {
        ...
        ClientPacketDistributor.sendToServer(new CartSizePayload(target.getId()));
    }
