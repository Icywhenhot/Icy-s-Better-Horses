...
// GeckoLib 5 widened GeoObjectRenderer from one type parameter to three: <T animatable, O relatedObject, R renderState>. We don't need a related object, so we use Void, and the default GeoRenderState.Impl suffices for the state.
public final class HorseStabilizerGeoRenderer
    ...
    public void adjustRenderPose(RenderPassInfo<GeoRenderState.Impl> renderPassInfo) {
        // The layer already anchors the stabilizer to the horse body. GeoObjectRenderer's default +0.5/+0.51/+0.5 translation is for standalone objects and pushes the rig off the horse.
    }
