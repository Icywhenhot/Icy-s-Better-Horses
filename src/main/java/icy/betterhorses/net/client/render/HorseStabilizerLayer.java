...
// Stabilizer wing layer for 1.21.11 + GeckoLib 5. Since the live horse is unreachable in RenderLayer.submit, IBhEquineStabilizerState carries the per-frame data and BhRenderContext carries the CameraRenderState; this class consumes both, anchors the wings to the body bone, then calls GeoObjectRenderer.performRenderPass.
public final class HorseStabilizerLayer<S extends EquineRenderState, M extends EntityModel<? super S>>
    ...
    public void submit(
        ...
        if (camera == null) {
            // Outside a LivingEntityRenderer.submit scope (shouldn't happen for a horse layer); defensively skip rather than NPE.
            return;
        }
        ...
        // GeckoLib 5 entry point: the renderer builds its own GeoRenderState; we just pass camera + collector + light. partialTick is typed int here and timing is driven by the AnimationController's clock, so 0 is safe.
