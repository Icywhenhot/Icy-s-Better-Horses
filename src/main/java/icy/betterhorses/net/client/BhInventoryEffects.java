...
public final class BhInventoryEffects {
    ...
    public static boolean fits(AbstractContainerScreen<?> screen) {
        AbstractContainerScreenAccessor frame = (AbstractContainerScreenAccessor) screen;
        return screen.width - (frame.bh_leftPos() + frame.bh_imageWidth() + 2) >= 32;
    }

    public static void render(AbstractContainerScreen<?> screen, GuiGraphics gfx, Font font, int mouseX, int mouseY) {
        Minecraft mc = Minecraft.getInstance();
        AbstractContainerScreenAccessor frame = (AbstractContainerScreenAccessor) screen;
        int x = frame.bh_leftPos() + frame.bh_imageWidth() + 2;
        int room = screen.width - x;
        if (mc.player == null || room < 32) {
            return;
        }
        List<MobEffectInstance> effects = mc.player.getActiveEffects().stream().sorted().toList();
        if (effects.isEmpty()) {
            return;
        }
