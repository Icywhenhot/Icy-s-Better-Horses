...
public final class BhInventoryEffects {
    ...
    private static final ResourceLocation LARGE = ResourceLocation.withDefaultNamespace("container/inventory/effect_background_large");
    private static final ResourceLocation SMALL = ResourceLocation.withDefaultNamespace("container/inventory/effect_background_small");
    ...
    public static void render(AbstractContainerScreen<?> screen, GuiGraphics gfx, Font font, int mouseX, int mouseY) {
        ...
        for (MobEffectInstance effect : effects) {
            gfx.blitSprite(wide ? LARGE : SMALL, x, y, wide ? 120 : 32, 32);
            ...
            if (wide) {
                ...
                gfx.drawString(font, MobEffectUtil.formatDuration(effect, 1.0F, tickrate), x + 28, y + 16, 0x7F7F7F);
        ...
        if (hovered != null) {
            gfx.renderTooltip(font, List.of(name(hovered), MobEffectUtil.formatDuration(hovered, 1.0F, tickrate)),
                    Optional.empty(), mouseX, mouseY);
        }
    }

    private static Component name(MobEffectInstance effect) {
        MutableComponent out = effect.getEffect().value().getDisplayName().copy();
